...
GsplatLodStreamingTwo.attributes.add('lodUpdateDistance', { type: 'number', default: 0.5, title: 'LOD Update Dist' });
...
GsplatLodStreamingTwo.prototype.initialize = function () {
    ...
    this.minBudget = 0.5; // Starts at 500k

    this.currentBudget = this.minBudget;

    this.budgetClimbStep = 1.0; // Fast climb by 1M

    this.budgetDropStep = 1.5;  // Fast drop by 1.5M



    // Quality Bounds for Interpolation
...
GsplatLodStreamingTwo.prototype.evaluateDynamicPerformance = function (fps) {
    ...
    if (fps < this.fpsDropThreshold) {
        ...
        // Requires only 1 second of stability to push up quickly

        if (this.stableFrames >= 1 && this.currentBudget < this.maxSplatBudget) {

            this.currentBudget = Math.min(this.maxSplatBudget, this.currentBudget + this.budgetClimbStep);

            this.stableFrames = 0; // Reset to allow continuous fast climbing next second
        ...
        // In the sweet spot (30 - 45 fps). Remain stable.
...
GsplatLodStreamingTwo.prototype.updatePresetUI = function () {
    ...
    this.presetUI.innerText =
        ...
        'BUDGET      : ' + this.currentBudget.toFixed(1) + 'M \n' +
...
GsplatLodStreamingTwo.prototype.applyResolution = function () {
    ...
    this.app.graphicsDevice.maxPixelRatio = this.highRes ? Math.min(dpr, 2) : (dpr >= 2 ? dpr * 0.5 : dpr);
