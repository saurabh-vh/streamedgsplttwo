...
// --- REAL-TIME RESOLUTION SLIDER ---
GsplatLodStreamingTwo.attributes.add('resolutionScale', { type: 'number', default: 1.0, min: 0.1, max: 2.0, title: 'Resolution Scale Multiplier' });

// --- 2. DYNAMIC BUDGET SETTINGS ---
GsplatLodStreamingTwo.attributes.add('maxSplatBudget', { type: 'number', default: 15.0, title: 'Max Budget (Millions)' });
...
GsplatLodStreamingTwo.attributes.add('lodUpdateDistance', { type: 'number', default: 2.0, title: 'LOD Update Dist' });
GsplatLodStreamingTwo.attributes.add('lodUnderfillLimit', { type: 'number', default: 5, title: 'LOD Underfill Limit' });

// --- 3. RENDERING & DEBUG SETTINGS ---
...
GsplatLodStreamingTwo.prototype.initialize = function() {
    ...
    // Performance State
    this.isHighPerformanceMode = true; // Kept enabled by default
    this.mobileSafeBudgetLimit = 3.0; // Reverted back to a standard 3M cap for High Perf mode

    // Auto-Performance Dynamic State
    this.minBudget = 0.5; 
    this.currentBudget = this.minBudget; 
    this.budgetClimbStep = 1.0; 
    this.budgetDropStep = 1.5;  
    
    this.qualityBounds = {
        minBaseDist: 5, maxBaseDist: 35,
        minMultiplier: 4, maxMultiplier: 2 
    };
    ...
    this.on('destroy', function() { 
        if (this.presetUIContainer) this.presetUIContainer.remove(); 
    }, this);
};

// --- DYNAMIC CAPABILITY DETECTION LOOP ---
...
GsplatLodStreamingTwo.prototype.evaluateDynamicPerformance = function(fps) {
    let changed = false;
    let currentCeiling = this.isHighPerformanceMode ? this.mobileSafeBudgetLimit : this.maxSplatBudget;

    if (this.currentBudget > currentCeiling) {
        this.currentBudget = currentCeiling;
        changed = true;
    }
...
GsplatLodStreamingTwo.prototype.applyDynamicQuality = function() {
    ...
    let ceiling = this.isHighPerformanceMode ? this.mobileSafeBudgetLimit : this.maxSplatBudget;
    let ratio = (this.currentBudget - this.minBudget) / (ceiling - this.minBudget);
    ...
    this.app.scene.gsplat.lodRangeMin = 0;
    this.app.scene.gsplat.lodRangeMax = 5;
...
GsplatLodStreamingTwo.prototype.setupPresetUI = function() {
    this.presetUIContainer = document.createElement('div');
    Object.assign(this.presetUIContainer.style, {
    ...
    Object.assign(this.presetUI.style, {
        ...
        fontFamily: 'monospace', fontSize: '13px', borderRadius: '6px', 
        boxShadow: '0 2px 5px rgba(0,0,0,0.5)', whiteSpace: 'pre', lineHeight: '1.4', pointerEvents: 'none'
    });

    this.toggleButton = document.createElement('button');
    this.toggleButton.innerText = this.isHighPerformanceMode ? "High Perf: ON" : "High Perf: OFF";
    Object.assign(this.toggleButton.style, {
        padding: '10px',
        background: this.isHighPerformanceMode ? 'linear-gradient(135deg, rgba(200, 40, 60, 0.8), rgba(255, 255, 255, 0.2))' : 'linear-gradient(135deg, rgba(120, 40, 200, 0.75), rgba(255, 255, 255, 0.1))',
        backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)', 
        border: '1px solid rgba(255,255,255,0.3)', borderRadius: '8px', color: '#ffffff',
        cursor: 'pointer', fontFamily: 'sans-serif', fontWeight: 'bold', fontSize: '12px',
        textTransform: 'uppercase', boxShadow: '0 4px 6px rgba(0,0,0,0.2)', transition: 'all 0.2s ease-in-out'
    });

    var self = this;
    this.toggleButton.onclick = function() {
        self.isHighPerformanceMode = !self.isHighPerformanceMode;
        
        if (self.isHighPerformanceMode) {
            self.toggleButton.innerText = "⚡ High Perf: ON";
            self.toggleButton.style.background = 'linear-gradient(135deg, rgba(200, 40, 60, 0.8), rgba(255, 255, 255, 0.2))';
        } else {
            self.toggleButton.innerText = "⚡ High Perf: OFF";
            self.toggleButton.style.background = 'linear-gradient(135deg, rgba(120, 40, 200, 0.75), rgba(255, 255, 255, 0.1))';
        }
        
        self.evaluateDynamicPerformance(self.lastFps);
        self.applyAllSettings();
    };

    this.toggleButton.onmouseover = () => this.toggleButton.style.transform = 'translateY(-2px)';
    this.toggleButton.onmouseout = () => this.toggleButton.style.transform = 'translateY(0)';

    this.presetUIContainer.appendChild(this.presetUI);
    this.presetUIContainer.appendChild(this.toggleButton);
    document.body.appendChild(this.presetUIContainer);
...
GsplatLodStreamingTwo.prototype.updatePresetUI = function() {
    ...
    var autoStatus = this.autoPerformance ? "ON (" + this.lastFps + " FPS)" : "OFF (MANUAL)";
    let baseDist = this.gsplatGs ? this.gsplatGs.lodBaseDistance : 0;
    
    // Calculate current internal pixel ratio for UI feedback
    let currentDPR = window.devicePixelRatio || 1;
    let scaleModifier = this.isHighPerformanceMode ? 0.5 : 1.0;
    let basePixelScale = this.highRes ? Math.min(currentDPR, 2) : (currentDPR >= 2 ? currentDPR * 0.5 : currentDPR);
    let finalPixelRatio = basePixelScale * scaleModifier * this.resolutionScale;

    this.presetUI.innerText = 
        'DYNAMIC LOD : ' + autoStatus + '\n' +
        'BUDGET      : ' + this.currentBudget.toFixed(2) + 'M \n' +
        'BASE DIST   : ' + baseDist + '\n' +
        'PIXEL RATIO : ' + finalPixelRatio.toFixed(2) + 'x';
};

// --- EXISTING FUNCTIONS (Loaders & Settings) ---
...
GsplatLodStreamingTwo.prototype.setupCamera = function() {
    ...
    if (!this.cameraEntity) {
        ...
        this.cameraEntity.addComponent('camera', { clearColor: new pc.Color(1, 1, 1), toneMapping: pc.TONEMAP_LINEAR, nearClip: 0.02 });
...
GsplatLodStreamingTwo.prototype.setupOccluder = function() {
    ...
    orangeMat.diffuse = new pc.Color(0, 0, 0); orangeMat.emissive = new pc.Color(1, 0.5, 0); orangeMat.update();
    this.cube.render.meshInstances[0].material = orangeMat;
    this.cube.setLocalPosition(6, 1, -2); this.cube.setLocalScale(2, 2, 2); this.cube.enabled = this.occluder;
...
GsplatLodStreamingTwo.prototype.applyAllSettings = function() {
    ...
    g.radialSorting = this.isHighPerformanceMode ? false : this.radialSorting; 
    g.dataFormat = this.compact ? pc.GSPLATDATA_COMPACT : pc.GSPLATDATA_LARGE;
    g.renderer = this.isHighPerformanceMode ? 1 : this.renderer; 
...
GsplatLodStreamingTwo.prototype.applyResolution = function() {
    ...
    var scaleModifier = this.isHighPerformanceMode ? 0.5 : 1.0; 
    var basePixelScale = this.highRes ? Math.min(dpr, 2) : (dpr >= 2 ? dpr * 0.5 : dpr);
    
    // Combine standard logic, high perf modifier, and editor resolution slider
    this.app.graphicsDevice.maxPixelRatio = basePixelScale * scaleModifier * this.resolutionScale;
    this.app.resizeCanvas();
    this.updatePresetUI(); // Update UI to reflect live slider drag
};
...
GsplatLodStreamingTwo.prototype.applyEnvironment = function() {
    ...
    if (!preset) { this.app.scene.skybox = this.defaultSkybox || null; this.app.scene.envAtlas = this.defaultEnvAtlas || null; return; }
...
GsplatLodStreamingTwo.prototype.bindEvents = function() {
    // Listens for real-time editor slider changes
    this.on('attr:resolutionScale', this.applyResolution, this);
